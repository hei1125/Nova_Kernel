...
# Config Hikari (Sony Xperia Acro S) OC Kernel
cp arch/arm/configs/hikari_oc_defconfig .config;
...
cd zip-format;
zip -r NovaKernel-Hikari-OC.zip ./META-INF kernel.elf;

# Move Kernel to shared folder
cp NovaKernel-Hikari-OC.zip /media/sf_Desktop;

# Remove leftovers in zip-format folder
rm -f $kernel_dir/zip-format/kernel.elf;
rm -f $kernel_dir/zip-format/NovaKernel-Hikari-OC.zip;
