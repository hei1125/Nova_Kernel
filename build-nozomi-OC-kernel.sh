...
# Config Nozomi (Sony Xperia S) OC Kernel
cp arch/arm/configs/nozomi_oc_defconfig .config;
...
cd zip-format;
zip -r NovaKernel-Nozomi-OC.zip ./META-INF kernel.elf;

# Move Kernel to shared folder
cp NovaKernel-Nozomi-OC.zip /media/sf_Desktop;

# Remove leftovers in zip-format folder
rm -f $kernel_dir/zip-format/kernel.elf;
rm -f $kernel_dir/zip-format/NovaKernel-Nozomi-OC.zip;
